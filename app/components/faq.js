...
const faqs = [
  ...
  },
  {
    ...
    answer:
      "The UC Cup welcomes participants of all ages and skill levels. Whether you are a seasoned athlete or a novice looking to try something new, ther is an event for you. Some events may have age categories or specific rules, so be sure to check the event details for specific requirements.",
  },
  {
    id: 6,
  ...
  },
  {
    id: 7,
    question:
      "> How can I contact the UC Cup organizers for further assistance?",
    answer:
          ...
          <dl className="space-y-16 sm:grid sm:grid-cols-2 sm:gap-x-6 sm:gap-y-16 sm:space-y-0 lg:gap-x-10 ">
            {faqs.map((faq) => (
              <div key={faq.id} className=" ">
                <dt
                  className="text-base font-semibold leading-7 text-gray-900 hover:cursor-pointer"
                  onClick={() =>
                    setVisibility(visibility === faq.id ? null : faq.id)
                  }
                >
                  {faq.question}
                </dt>
